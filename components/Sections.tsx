...
export function Steps({ steps }: { steps: string[] }) {
  const stepMeta = [
    { icon: '📞', time: '2 min', color: '#E86C2F' },
    { icon: '🔍', time: '24 hrs', color: '#f59e0b' },
    { icon: '⚡', time: '7–14 days', color: '#10B981' },
    { icon: '📈', time: 'Ongoing', color: '#6366f1' },
  ];

  return (
    <div className="relative">
      {/* Connector line — desktop only */}
      <div className="absolute left-0 right-0 top-10 hidden border-t-2 border-dashed border-orange-200 md:block" style={{ zIndex: 0 }} />

      <div className="relative grid gap-6 md:grid-cols-4" style={{ zIndex: 1 }}>
        {steps.map((step, index) => {
          const [title, body] = step.split(' — ');
          const meta = stepMeta[index] ?? { icon: '✨', time: '', color: '#E86C2F' };
          return (
            <Reveal key={step} delay={index * 100}>
              <article className="group relative flex gap-5 rounded-2xl border border-gray-100 bg-white p-5 shadow-card transition hover:border-orange-100 hover:shadow-lg md:flex-col md:gap-0 md:text-left">
                {/* Number + icon */}
                <div className="relative flex-shrink-0">
                  <div
                    className="flex h-14 w-14 items-center justify-center rounded-2xl text-2xl font-bold text-white shadow-md transition group-hover:scale-105"
                ...
                <div className="md:mt-5">
                  <div className="mb-2 inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-widest" style={{ background: `${meta.color}15`, color: meta.color }}>
                    {meta.time}
                  </div>
                  <h3 className="font-heading text-lg font-bold text-[#1A1A2E]">{title}</h3>
      ...
      <Reveal delay={420}>
        <div className="mt-10 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
          <a
            href="/contact"
            className="inline-flex min-h-12 items-center justify-center rounded-full bg-[#E86C2F] px-8 py-3 font-heading font-semibold text-white shadow-md transition hover:bg-orange-600 hover:shadow-lg"
          >
            Book My Free Call →
          </a>
          <a
            ...
            className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border-2 border-[#25D366] px-8 py-3 font-heading font-semibold text-[#25D366] transition hover:bg-[#25D366] hover:text-white"
          >
